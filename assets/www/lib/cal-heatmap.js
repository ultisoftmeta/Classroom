...
var CalHeatMap = function() {
	...
	this._domainType = {
		...
		"hour" : {
			name: "hour",
			row: function(d) {return 24;},
			column: function(d) {
				switch(self.options.domain) {
					case "day" : return 1;
					case "week" : return 7;
					case "month" : return self.getEndOfMonth(d).getDate();
				}
			},
			position: {
				x : function(d) {
					if (self.options.domain === "month") {
						return Math.floor(d.getHours() / self._domainType.hour.row(d)) + (d.getDate()-1);
					} else if (self.options.domain === "week") {
						return Math.floor(d.getHours() / self._domainType.hour.row(d)) + self.getWeekDay(d);
					}
